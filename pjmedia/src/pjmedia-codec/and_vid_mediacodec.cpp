...
/* This callback is useful if some additional configuration is needed for 
 * specific codec.
 */
typedef void(*configure_decoder_cb)(anmed_codec_data *anmed_data,
...
static void configure_decoder_avc(anmed_codec_data *anmed_data,
                                  AMediaFormat *vid_fmt);
...
static struct anmed_codec {
    ...
    open_cb            open_codec;
    configure_decoder_cb configure_decoder;
...
static pj_status_t configure_decoder(anmed_codec_data *anmed_data) {
    ...
    AMediaFormat_setInt32(vid_fmt, ANMED_KEY_PRIORITY, 0);
    if (anmed_codec[anmed_data->codec_idx].configure_decoder) {
        anmed_codec[anmed_data->codec_idx].configure_decoder(anmed_data,
                                                             vid_fmt);
    }
